...
from latentvelo.dataloader_nogcn import DatasetNoGCN
...
def train_vae_nogcn(model, adata, epochs = 50, learning_rate = 1e-2, batch_size = 200, grad_clip = 1, shuffle=True, test=0.1, name = '', optimizer='adam', random_seed=42):
    ...
    dataset = DatasetNoGCN(adata, shuffle=shuffle, test=test, random_seed=random_seed)
    ...
    for epoch in range(epochs):
        ...
        with th.no_grad():
            ...
            print('epoch %d, full loss %.3f, val loss %.3f, recon MSE %.3f, traj MSE %.3f, reg loss %.3f'%(epoch,
                                                                                 loss,
